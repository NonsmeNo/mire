...
process(Stream, Words) :-
  (member(action, Words) ->  % Проверяем наличие ключевого слова 'action'
    format(atom(Command), 'perform_action~n', []);  % Отправляем соответствующую команду
  member(another_action, Words) ->
    format(atom(Command), 'perform_another_action~n', []);
  true ->  % Добавьте другие обработки для других ключевых слов
    format(atom(Command), 'move forward~n', []) % По умолчанию делаем движение вперед
  ),
  write(Command),
  write(Stream, Command),
  flush_output(Stream).
